...
__all__ = ["Shape", "balanced", "semibalanced",
           "Evaluator", "hcp", "fibhcp", "hsp", "qp", "controls",
           "Rank", "A", "K", "Q", "J", "T",
...
fibhcp = Evaluator(21,13,8,5,3)
fibhcp.__name__ = "fibhcp"
...
class Deal(tuple):
    ...
    def __new__(cls, dealer, accept_func=None, tries=1000000):
        """
...
class Hand(tuple):
    ...
    shape = util.reify(lambda self: tuple(len(holding) for holding in self),
                       "The hand's shape.")
    fibhcp = util.reify(lambda self: sum(holding.fibhcp for holding in self),
                     "The hand's Fibonacci HCP count.")
    ...
    hsp = util.reify(lambda self: sum(holding.hsp for holding in self), 
                     "The holding's HSP")
...
class Holding(frozenset):
    ...
    fibhcp = util.reify(fibhcp, "The holding's Fibonacci HCP.")
    qp = util.reify(qp, "The holding's QP.")
    hsp = util.reify(hsp, "The holding's HSP")
    ...
    ## Added functionality for Honors Sequence Points.
    @util.reify
    def hsp(self):
        """
        The holding's Honors Sequence Points
        """
        if {A, K, Q, J, 10} <= self:
            return 10
        if {A, K, Q, J} <= self or {K, Q, J, 10} <= self: 
            return 6
        if {A, K, Q} <= self or {K, Q, J} <= self or {Q, J, 10} <= self:
            return 3
        if {A, K} <= self or {K, Q} <= self or {Q, J} <= self or {J, 10} <= self:
            return 1
        return 0
